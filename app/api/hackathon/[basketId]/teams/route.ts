...
export async function POST(req: Request, ctx: { params: Promise<{ basketId: string }> }) {
  ...
  const isTeamLeader = async (teamId: string) => {
    const { data: team } = await sb
      ...
      .maybeSingle();
    return !!team && (team.leader_user_id === identity.userId || team.leader_user_id === identity.email);
  };

  // Takım adını/açısını organizatör dışında sadece o takımın (rastgele atanan) lideri değiştirebilir.
  if (body.action === "rename") {
    const { teamId, name } = body as Extract<Action, { action: "rename" }>;
    if (!canManage && !(await isTeamLeader(teamId))) {
