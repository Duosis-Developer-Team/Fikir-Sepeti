...
export function InvitePanel({ basketId }: { basketId: string }) {
  ...
  const whatsapp = () => {
    const text = encodeURIComponent(`FikirSepeti daveti — lobiye katıl: ${url}`);
    window.open(`https://wa.me/?text=${text}`, "_blank", "noopener,noreferrer");
  };

  return (
    <div className="grain relative overflow-hidden rounded-[24px] p-7" data-testid="invite-panel" style={{ background: "linear-gradient(180deg, var(--sheen), transparent 46%), var(--card)", border: "1px solid rgba(231,169,63,0.28)", boxShadow: "var(--card-shadow), 0 30px 60px -42px rgba(231,169,63,0.3), inset 0 1px 0 var(--edge)" }}>
      ...
      <div className="mt-4 flex items-center gap-4">
        {qr ? <img src={qr} alt="davet QR" className="h-[92px] w-[92px] shrink-0 rounded-xl" style={{ background: "#fff", padding: 6 }} data-testid="invite-qr" /> : <div className="h-[92px] w-[92px] shrink-0 rounded-xl" style={{ background: "var(--surface-2)" }} />}
      ...
      <div className="mt-4 flex items-center gap-2 rounded-xl px-3 py-2.5" style={{ background: "var(--card)", border: "1px solid rgba(var(--border-rgb),0.08)" }}>
        <span className="min-w-0 flex-1 truncate text-[0.85rem]" style={{ color: dim(0.65) }} data-testid="invite-url">{url}</span>
        <button
          onClick={copy}
      ...
      <button
        type="button"
        onClick={whatsapp}
        className="mt-3 w-full rounded-full px-4 py-2.5 text-[0.9rem] font-semibold"
        style={{ background: "rgba(37,211,102,0.18)", color: "#6FD9B4" }}
